...
def select_features(forest: RandomForest, train_data: DataSet, validation_data: DataSet) -> List[float]:
    main_error = forest.oob_error(train_data)
    print('OOB error: {}'.format(main_error))
    res = []
    for i in range(validation_data.features_num):
        shuffled = shuffle_feature(validation_data, i)
